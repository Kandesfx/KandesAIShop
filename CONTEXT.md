...
| D65 | API gateway script install cho khách hàng (Codex/Claude Code config) | 🔜 In progress (2026-08-09) | User muốn khi khách mua AI API key, họ copy-paste 1 lệnh tự động config Codex/Claude Code để endpoint URL trỏ về `api.kandes.shop` thay vì `api.ccpro.cn` (nhà cung cấp). Luồng: khách copy lệnh `iex(irm https://kandes.shop/install/codex/codex-config-api.ps1 | iex)` → script tải về → tự động sửa file config của Codex/Claude Code → từ giờ AI request đi qua `api.kandes.shop` → EC2 proxy sang `api.ccpro.cn`. Cần: (1) route `api.kandes.shop` qua Route 53 CNAME → EC2 IP; (2) tạo 2 script install (PowerShell cho Windows, Bash cho Mac/Linux) tương tự ccpro.cn; (3) host script trên `kandes.shop/install/codex/...` hoặc S3 + CloudFront. User sẽ cung cấp script mẫu ccpro.cn để tham khảo. |
