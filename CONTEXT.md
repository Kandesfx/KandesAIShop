...
| Phase 9: Storefront Polish & UX Enhancement | 🔄 **In Progress** (bắt đầu 2026-08-09) | **2/15 features done**. 15 features chia 4 đợt (7-10 ngày). **Đợt 1 (PDP Polish, 3 ngày)**: ✅ D10 average rating (`bcf96f3`) — StarRating component + Product.avgRating/reviewCount + seed data + type conversion (Decimal→number). ✅ D1 Q&A tabs (`2836a93`) — ProductQuestion migration + product-question module (types/repository/service/validators) + 5 API routes (public list, user submit, admin manage) + ProductDetailTabs/QuestionList/AskQuestionForm components + Textarea UI. ⏳ D1 Reviews tab (pending), D2 cross-sell carousel, D4 share buttons, D5 trust signals. **Đợt 2 (Checkout Hardening)**: C7 Turnstile CAPTCHA + D7 checkout timeline + C5/F4 success page route + F3 poller optimization. **Đợt 3 (Cart & Mobile UX)**: D3 save-for-later/wishlist + D8 cart icon bounce + D6 mobile gallery scroll. **Đợt 4 (Technical)**: C6 countdown sync tabs + D9 footer dynamic version + cleanup. Kết quả hiện tại: 450/450 tests pass, typecheck + lint clean. Spec: `docs/tasks/PHASE_9_POLISH.md`. Plan: `docs/tasks/PHASE_9_PLAN.md`. |
