...
| Phase 8b: Storefront Purchase Flow UI Cleanup | ✅ Done (2026-08-07) | Audit storefront (Landing → Catalog → PDP → Cart → Checkout → Order → Track) → 47 findings (A1..F6) triển khai xong qua 5 đợt ưu tiên. **Đợt 1** unblock purchase flow (A1 AddToCartButton + A5 variant selector + A6 bỏ name state + A3 router.refresh + B10 footer link + C10 stale comment). **Đợt 2** filter UX (A2 sticky bar + B2 clearAll URL + C3 form labels + F1 meta description). **Đợt 3** mobile + UX polish (B1 mobile-nav + B3 sticky offset + B4 empty state clearAll + B5 PDP gallery `<Image>` + B8/B9 OrderStatusBadge + RevealKeyDialog + C1 getMinProductPrice helper + C2 shared badge + C4 logger + C8 NavLink active route). **Đợt 4** accessibility (E1 focus trap + E2 countdown announce + E3 aria-pressed + E4 semantic dl/dt/dd + E5 form labels). **Đợt 5** contract hardening (A4 error context + C5 OrderStatusPoller local state + D9 build ID + F3 early-return terminal + F6 jsonLd + D1-D10 Phase 9 backlog). Verify: typecheck + lint (1 warning) + test (447/447) + build (Compiled successfully). Spec: `docs/tasks/PHASE_8_STOREFRONT_CLEANUP.md`. Plan: `docs/tasks/PHASE_8_PLAN.md`. Không có deviation mới (D65+). |
| Phase 8c: Landing Hero Redesign | ✅ Done (2026-08-08) | User yêu cầu "trùng tu" hero — bỏ glitch/scanline/RGB-shift cyber AI-generic style, dùng video background đã chuẩn bị sẵn (`public/assets/video/bg-video-compressed.webm`) với poster SVG fallback. Layout mới: centered hero + 2 gradient CTA cards (purple→cyan "AI Gateway" → `/products?category=ai-code`; orange→red "Mua ngay" → `/products`) + trust strip. Bỏ: `<IntroLogo>` GIF (phản cảm), rotating ticker, scanlines grid, RGB-shift animation, stats terminal grid, frame brackets. Tạo `components/brand/video-background.tsx` (reusable, prefers-reduced-motion fallback) + `components/sections/featured-products.tsx` (extract từ `app/page.tsx`). Refactor `app/page.tsx` section order: Hero → Featured Products (grid 1/2/3 cols ngay dưới hero — user yêu cầu) → Categories (renumbered [03]) → ValueProps (renumbered [04]). Update section numbers [02/03/04]. Thêm Tailwind tokens `sunset` (orange-red) + 2 gradient backgrounds `gradient-ai-gateway` + `gradient-buy-now`. Verify: typecheck + lint (1 pre-existing warning) + test (447/447) + build (✓ Compiled successfully). |
...
| D65 | Brand extension: thêm token `sunset` (orange-red) + 2 gradient backgrounds cho hero CTA (Phase 8c) | ✅ Accepted (2026-08-08) | User share reference image 2026-08-08 với 2 CTA card có gradient cam-vàng → đỏ ("Mua ngay"). Tailwind tokens hiện tại chỉ có `electric` (cyan) + `plasma` (purple). Thêm `sunset` (FF6A2C default, E03E1A deep) + `gradient-ai-gateway` (purple→indigo→cyan) + `gradient-buy-now` (yellow→orange→red). Chỉ dùng cho hero CTA cards, không thay đổi brand identity chính (electric vẫn là primary). |
| D66 | VideoBackground component — reusable, prefers-reduced-motion fallback (Phase 8c) | ✅ Accepted (2026-08-08) | `components/brand/video-background.tsx` — wrap `<video autoplay muted loop playsInline>` với poster SVG fallback + dark overlay. Tôn trọng `prefers-reduced-motion` (D34 pattern): user reduce → chỉ poster + overlay, skip video. Source chain: webm trước (nhỏ hơn), mp4 fallback cho Safari cũ. Props: `sources`, `poster`, `overlay` ('none'/'soft'/'strong'), `reducedFallback`, `ariaLabel`. Sử dụng file `public/assets/video/bg-video-compressed.webm` (12MB — đã có sẵn, chưa được dùng từ Phase 0) + `public/assets/brand/hero-poster.svg` (poster mới tạo). |
| D67 | Phase 8c: Featured Products chuyển xuống ngay dưới Hero (Phase 8c) | ✅ Accepted (2026-08-08) | User yêu cầu "trình bày trước các sản phẩm ở dưới". Refactor `app/page.tsx` section order: Hero → FeaturedProducts (grid 1/2/3 cols responsive, ngay dưới hero) → Categories → ValueProps. Extract inline featured section từ `app/page.tsx` thành `components/sections/featured-products.tsx` component. Section numbers renumbered: Featured `[02/...]`, Categories `[03/...]`, ValueProps `[04/...]` (Hero không có prefix — minimal aesthetic). |
