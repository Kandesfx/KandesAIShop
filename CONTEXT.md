...
| Phase 8: UI Cleanup + Re-enable CI gates | 🔜 Up next (after CD setup) | Sửa các UI/system lỗi bạn phát hiện. Sau khi codebase clean → re-enable lint + test trong CD workflow. |

## 3. Bắt đầu 1 task — Checklist (BẮT BUỘC)

1. Đọc `docs/tasks/PHASE_X_*.md` cho task hiện tại.
...
| D61 | CI/CD: GitHub Actions (self-hosted runner) — workflow `.github/workflows/deploy-prod.yml` | ✅ Accepted (2026-08-06) | GitHub-hosted runner của repo này không available (queue stuck + push events không trigger). Workaround: cài self-hosted runner trên EC2 (label `kandes`), dùng OIDC `kandes-github-deploy` role với policy ECR push. Workflow gồm 2 job: `build` (npm ci + typecheck + build, **lint/test skipped** vì codebase chưa clean — cần fix sau) + `deploy` (migrate + restart containers + health check + smoke). Trigger: push main + workflow_dispatch. 8 GitHub Secrets: AWS_DEPLOY_ROLE, ECR_REGISTRY, EC2_HOST, EC2_SSH_KEY, PROD_DOMAIN, DATABASE_URL, TELEGRAM_BOT_TOKEN, TELEGRAM_ADMIN_CHAT_ID. Known issue: lint/test fail vì codebase chưa fix — Phase UI cleanup sẽ xử lý. |
