...
## 2. Phase status (cập nhật 2026-08-11)

> **Deployment milestone (2026-08-11):** D71 (orphan EIP) + D72 (cron) + D73 (RDS) deployed. Auto-stop/start DEFERRED (user decision: 24/7). See §7 D60..D73.
...
| Phase 8b: Storefront Purchase Flow UI Cleanup | ✅ Done (commits `96f3e4a..89e9f36`, 2026-08-09) | Audit storefront (Landing → Catalog → PDP → Cart → Checkout → Order → Track) → 47 findings (A1..F6) chia 5 đợt. **47/47 findings implemented (100%)**. Đợt 1 (`c1d0e43`): A1 AddToCartButton + A5 variant selector + A3 router.refresh + B10 footer link + C10 stale comment. Đợt 2 (same): CartProvider context (useReducer) + auth-aware header. Đợt 3 (same): Mobile nav drawer + sticky filter offset + PDP gallery + UX polish. Đợt 4 (`3797253`): Accessibility (E1-E5) — focus trap hook, countdown ARIA, variant aria-pressed, list semantics, useId. Đợt 5 (`db3687e`): Contract hardening (A4 checkout error context, D9 dynamic version, F3/F6 comments). **F1 fix (`89e9f36`)**: itemCount vs lineCount clarification in cart UI. Kết quả: 53/53 test files, 447/447 tests pass, typecheck + lint clean (1 warning pre-existing). Audit report: `docs/tasks/PHASE_8B_AUDIT_REPORT.md`. Spec: `docs/tasks/PHASE_8_STOREFRONT_CLEANUP.md`. Plan: `docs/tasks/PHASE_8_PLAN.md`. |
| Phase 9: Storefront Polish & UX Enhancement | ✅ Done (2026-08-09) | **15/15 features done**, 4/4 đợt hoàn tất. **Đợt 1 (PDP Polish)**: D10 average rating (StarRating + Product.avgRating/reviewCount) · D1 Reviews & Q&A tabs (ProductQuestion module + ReviewsTab/WriteReviewForm/ProductDetailTabs) · D2 cross-sell carousel (giá ±30% + fallback) · D4 share buttons (FB/Twitter/copy/native share) · D5 trust signals block. **Đợt 2 (Checkout Hardening)**: C7 Turnstile CAPTCHA (fail-open nếu chưa config/Cloudflare down) · D7 CheckoutTimeline stepper · C5+F4 success page route `/order/[id]/success` + `OrderDetailView` shared component · F3 OrderStatusPoller early-return cho terminal status + fix redirect bug (cancelled không nhảy sang success). **Đợt 3 (Cart & Mobile UX)**: D3 Wishlist/"Lưu lại sau" (migration `wishlists` table + module + 3 API routes + `/account/wishlist` page, guest→login prompt) · D8 cart icon bounce animation (chỉ khi count tăng) · D6 mobile gallery thumbnails horizontal scroll (snap-x, desktop giữ grid). **Đợt 4 (Technical)**: C6 countdown sync qua BroadcastChannel (drift >500ms đồng bộ lại) · D9 footer build ID dynamic (`{year}.Q{quarter}.PHASE-9`, override qua `NEXT_PUBLIC_BUILD_ID`) · cleanup 4 route handlers `console.error`→`logger.error`. Kết quả: 504/504 tests pass (+57 từ baseline 447), typecheck + lint clean (1 warning pre-existing `no-img-element`), build pass (58/58 routes). Spec: `docs/tasks/PHASE_9_POLISH.md`. Plan: `docs/tasks/PHASE_9_PLAN.md`. Progress: `docs/tasks/PHASE_9_PROGRESS.md`. |
| Phase 10+ (backlog) | ⏳ | Admin panel enhancements, AI Gateway UI polish, Multi-language i18n, SEO optimization, Performance optimization. |

## 3. Bắt đầu 1 task — Checklist (BẮT BUỘC)

1. Đọc `docs/tasks/PHASE_X_*.md` cho task hiện tại (Phase 9 bắt đầu → `PHASE_9_POLISH.md` + `PHASE_9_PLAN.md`).
...
| D62 | Upgrade instance: t3.micro → m7i-flex.large (8 GB RAM, 2 vCPU Intel) | ⏸️ **REVISED** (2026-08-10) | Plan đã accept nhưng **chưa bao giờ thực sự migrate**. Thực tế tại 2026-08-10 instance đang chạy **`t3.small`** (2 GB RAM, chứ không phải `m7i-flex.large` 8 GB như plan). Cost estimate updated: $19/mo thay vì $89.7/mo. Lý do có thể: (a) rollback thủ công sau khi quan sát burn rate, hoặc (b) D66 build/deploy split (2026-08-10) làm giảm urgency vì build không còn cần RAM lớn trên EC2 nữa. Action: nếu user muốn migrate thật → chạy `scripts/aws/migrate-instance.sh` (sau khi fix hardcoded `OLD_TYPE` t� `t3.micro` → `t3.small`). Xem `docs/deployment/CURRENT_STATE.md` §1. |
| D62a | Thực tế: instance đã ở `t3.small` từ launch (2026-08-06) | ✅ Accepted (2026-08-10) | Launch time `2026-08-06T17:48:26+07:00`, instance type `t3.small`, đã chạy ~3.5 ngày ổn định. CPU 1-3% trong 24h quan sát → heavily over-provisioned cho traffic hiện tại. Có thể hạ xuống `t3.micro` (1 GB, Free Tier) nếu cần cost saving thêm (~$10/mo), nhưng cần D66 fix để build không OOM trên 1 GB. |
| D63 | Cloudflare proxy ở front (CDN + WAF + bandwidth savings) | �️ DEFERRED (2026-08-09) | Sau khi phân tích kỹ (xem `docs/deployment/DECISION_LOG.md` §8), user quyết định HỦY Cloudflare setup. CloudFront + EC2 hiện tại đủ handle 10-50 users streaming (EC2 chỉ proxy/forward, không xử lý AI). Latency 30-80ms hiện tại chấp nhận được. Cloudflare thêm vào giữa user và CloudFront chỉ làm CHẬM thêm. Nếu sau này cần optimize → có thể REPLACE CloudFront bằng Cloudflare (không phải thêm vào). Cấu hình cũ giữ trong `docs/deployment/CLOUDFLARE_SETUP.md` để tham khảo. |
| D64 | Safety mechanism: budget cap + alarms (auto-stop DEFERRED) | 🟡 **PARTIAL** (2026-08-10) | Budget alarms + CloudWatch alarms active. Auto-stop/start **DEFERRED** (user decision: 24/7). |
| D65 | API gateway script install cho khách hàng (Codex/Claude Code config) | 🔜 In progress (2026-08-09) | User muốn khi khách mua AI API key, họ copy-paste 1 lệnh tự động config Codex/Claude Code để endpoint URL trỏ về `api.kandes.shop` thay vì `api.ccpro.cn` (nhà cung cấp). Luồng: khách copy lệnh `iex(irm https://kandes.shop/install/codex/codex-config-api.ps1 | iex)` → script tải về → tự động sửa file config của Codex/Claude Code → từ giờ AI request đi qua `api.kandes.shop` → EC2 proxy sang `api.ccpro.cn`. Cần: (1) route `api.kandes.shop` qua Route 53 CNAME → EC2 IP — **✅ DONE** (Route 53 hiện có A record `api.kandes.shop` → `13.215.39.207`, TTL 300); (2) tạo 2 script install (PowerShell cho Windows, Bash cho Mac/Linux) tương tự ccpro.cn — pending user input mẫu; (3) host script trên `kandes.shop/install/codex/...` hoặc S3 + CloudFront — pending. |
| D66 | Build/Deploy split (GitHub-hosted build + EC2 self-hosted deploy) | ✅ Accepted (2026-08-10) | Trước D66, workflow `.github/workflows/deploy-prod.yml` chạy **toàn bộ build trên self-hosted runner** (chính EC2 của user). Next.js build tốn 3-5 GB RAM peak. D66 tách build (GitHub-hosted ubuntu-latest, 16 GB RAM FREE) → push GHCR (`ghcr.io/kandesfx/kandesaishop`) → EC2 (self-hosted) chỉ `docker pull` + restart. Files: `Dockerfile` (multi-stage deps→builder→runner, ~150 MB final), `.dockerignore`, `next.config.js` (`output: 'standalone'`), rewrite `.github/workflows/deploy-prod.yml`. Hệ quả: EC2 chỉ cần runtime RAM ~800 MB → có thể hạ xuống `t3.small`/`t3.micro` mà không OOM. Trade-off: GHCR image public mặc định (image KHÔNG chứa secret — `.env` bị `.dockerignore` loại); GitHub Actions 2000 min/mo free, build ~5-8 min → ~250-400 builds/mo OK. Xem `docs/deployment/DECISION_LOG.md` §9 + `EC2-SETUP.md`. |
| D67 | Silent downgrade: m7i-flex.large → t3.small (chưa documented) | ⚠️ **Implicit** (2026-08-10) | State thật tại 2026-08-10 cho thấy instance đang chạy `t3.small`, không phải `m7i-flex.large` như D62. Không có deviation D67 chính thức — có thể là rollback ngầm sau D66 (vì build OOM không còn là vấn đề). User confirm hay không confirm: cần hỏi. Cost: $19/mo thay vì $89.7/mo (~$70/mo saving). Nếu user confirm D67: update D62 status thành ⏸️ REVISED + thêm D67 chính thức. Xem `CURRENT_STATE.md` §1 + `DECISION_LOG.md` §10. |
| D68 | Idle-trigger stop + Health-check wake | ⏸️ **DEFERRED** (user decision: 24/7) | Route 53 health check `kandes-wake-20260809` đã có. Lambda wake/deploy scripts vẫn giữ trong `scripts/aws/` để tham khảo. |
| D69 | Cron jobs Vercel ngừng sau EC2 migration | ✅ **FIXED** (D72, 2026-08-11) | EventBridge + Lambda proxy thay thế Vercel cron. Xem D72. |
| D70 | DB conflict: RDS + container `db` | ✅ **FIXED** (D73, 2026-08-11) | Container `db` COMMENTED trong docker-compose.yml. RDS SG verified: only port 5432 from EC2 SG. Xem D73. |
| D71 | Orphan Elastic IP `18.140.102.23` (eipalloc-077dae100efe47c86) | ✅ **RELEASED** (2026-08-11) | EIP đã release khỏi AWS account. Không còn tốn phí. |
| D72 | EventBridge + Lambda cron proxy (Option B) | ✅ **DEPLOYED** (2026-08-11) | Lambda `kandes-cron-proxy` (Node.js 20.x, 256 MB) + IAM role `kandes-lambda-cron-proxy` + Secrets Manager secret `kandes/cron-secret` (48-byte base64). 5 EventBridge rules ENABLED: `kandes-cron-sepay-reconcile` (*/5min), `kandes-cron-expire-overdue-orders` (hourly), `kandes-cron-sla-scan` (*/5min), `kandes-cron-ai-balance-sync` (*/30min), `kandes-cron-ai-quota-alert` (6h). CRON_SECRET added to `/opt/kandes/.env.production` on EC2 + container verified loaded. Lambda curl test → `{"ok":true}`. Cost ~$0.45/mo. Scripts: `scripts/aws/setup-cron-schedule.sh`. Docs: `docs/deployment/CRON_SETUP.md`. Pending: auto-stop/start Lambda (D64/D68). |
| D73 | DB consolidation: RDS primary + container `db` removed | ✅ **DEPLOYED** (2026-08-11) | Container `db` COMMENTED in docker-compose.yml. RDS SG `sg-0674245896ecd632d` verified: only port 5432 allowed from EC2 SG `sg-0748ca482f2f9f073`. DATABASE_URL points to RDS endpoint. Scripts: `scripts/aws/setup-rds-primary.sh`. Docs: `docs/deployment/RDS_SETUP.md`. |
| D74 | Production critical-bug sweep (2026-08-11) | ✅ Accepted (2026-08-11) | Audit (D62a evidence) chỉ ra 4 bugs critical trong production: (1) docker-compose `wget` healthcheck nhưng image không có wget → container `unhealthy` 395 lần; (2) docker-compose chỉ mount `.env` (4 keys), KHÔNG mount `.env.production` chứa `CRON_SECRET`, AWS_*, EMAIL_PROVIDER → 5 cron jobs + backup-db + email OTP fail silent. Fix: healthcheck → `curl`; mount symlink `.env.kandes → .env.production` (mode 0644 cho container UID 1001) qua `scripts/aws/setup-env-link.sh`; (3) `db-backup` cron orphan — file `backup-db.ts` tồn tại nhưng không register registry, code dùng `S3_BUCKET` env thiếu. Fix: register 'db-backup' vào registry, JobHandler adapter, env chain `AWS_S3_BUCKET → S3_BUCKET → R2_BUCKET`, Dockerfile `+postgresql-client` cho `pg_dump`, EventBridge rule mới `cron(0 3 * * ? *)`; (4) Resend email provider chỉ stub — implement thật (Resend REST API) + ResendEmailProvider fail-fast nếu missing key. SES giữ stub cho D75+. Files changed: `docker-compose.yml`, `Dockerfile`, `modules/jobs/{types,registry,backup-db,tests/jobs.test}.ts`, `lib/email.{ts,test.ts}`, `scripts/aws/{setup-ec2-d66,setup-cron-schedule,setup-env-link}.sh`. Test: 439/439 pass (55 files, loại integration thiếu DB). |
| D75 | ACM cert `*.kandes.shop` (planned) | 🔜 Planned (2026-08-11) | `api.kandes.shop` Route 53 A record → EC2 EIP ok, nhưng PowerShell `Invoke-WebRequest` fail `SSL/TLS trust` (CERTIFICATE_AUTHORITY_INVALID). Nguyên nhân: ACM cert ở us-east-1 = empty → CloudFront không có cert custom cho `api.kandes.shop`. Action: request ACM cert us-east-1 cover `*.kandes.shop` + `kandes.shop` (DNS validation qua Route 53 NS đã có), tạo CloudFront distribution mới cho `api.kandes.shop` (price-class 100, ACM cert, origin = EC2 ALB hoặc trỏ thẳng EIP), hoặc dùng ACM cert cho Nginx trên EC2 trực tiếp (Let's Encrypt path đã có trên `kandes.shop` qua CloudFront — replicate cho subdomain). Pending user approval trước khi tạo AWS resources (cost ~$0-5/mo cho 1 cert + 1 distribution). |
| D75a | CloudFront distribution #2 cho `api.kandes.shop` | ✅ **DEPLOYED** (2026-08-12) | Phát hiện: cert ACM `arn:aws:acm:us-east-1:870730509911:certificate/8298c974...` đã cover `kandes.shop` + `*.kandes.shop` (sẵn cho distribution #1). Tạo CloudFront distribution #2 `EP5ZI3VMCBDP3` (domain `d2kdrnpwhtm31f.cloudfront.net`, alias `api.kandes.shop`, origin `ec2-origin.kandes.shop` A record → 13.215.39.207, ViewerProtocolPolicy `redirect-to-https`, no-cache passthrough, price-class 100). Update Route 53: `api.kandes.shop` A record từ plain `13.215.39.207` → CloudFront alias `d2kdrnpwhtm31f.cloudfront.net`. Tạo thêm A record `ec2-origin.kandes.shop` → EIP (vì CloudFront origin không nhận IP trực tiếp). Files: `scripts/aws/cf-api-dist-config.json`. Cost: ~$0.50-1/mo cho distribution #2 (cert free). |
| D76 | Cleanup sweep (D74 batch A–F) | ✅ **DONE** (2026-08-12) | (A) All 13 Prisma migrations applied to production RDS, 6/6 cron jobs pass (`sepay-reconcile`, `sla-scan`, `ai-balance-sync`, `ai-quota-alert`, `expire-overdue-orders`, `db-backup`). Fixed 3 latent migration bugs: `(1) 20260805030000_add_user_notification_prefs` dùng `"user"` thay vì `"users"`, `(2) 20260805150000_add_ai_api_key_rotation_policy` dùng `UUID` thay vì `TEXT` (FK to `ai_ncc_keys.id` text), `(3) duplicate migration 20260809041000_add_product_questions`. (B) Cron backup rclone Google Drive — `rclone 1.74` installed trên EC2, `/etc/cron.d/kandes-db-backup` 03:30 UTC daily, `scripts/aws/backup-db-rclone.sh` ready, `docs/deployment/BACKUP_RCLONE.md`. User cần chạy `rclone config` OAuth Google Drive. (C) CloudFront `api.kandes.shop` — distribution #2 `EP5ZI3VMCBDP3` deployed. Origin `ec2-origin.kandes.shop` → EIP, OriginProtocolPolicy `http-only`, EC2 Nginx stub server block `server_name api.kandes.shop` (port 80, no HTTPS redirect). Verified `curl https://api.kandes.shop/api/health` → **HTTP 200 SSL ok**. Route 53: `api.kandes.shop` A (alias) → CF#2, `ec2-origin.kandes.shop` A → EIP. (D) Xóa 4 secrets unused: `kandes/JWT_SECRET`, `kandes/REFRESH_SECRET`, `kandes/NEXTAUTH_SECRET`, `kandes/NEXTAUTH_URL`. Giữ `kandes/AWS_S3_BUCKET` (vẫn dùng trong D74 backup-db). (E) Xóa ECR repo orphan `kandes-shop` (empty, unused từ D66 migrate sang GHCR). (F) Sửa CI GitHub Actions — 5 commits (`0ba72a3` F, `5c0613f` F2 idempotent migrations, `7d576b3` F3 drop schema, `3ca9602` F4 pgcrypto ext, `966c0b7` F5 TEXT FK types). `gen_random_uuid()::text` + `IF NOT EXISTS` guards. Production RDS dùng TEXT ids cho support_tickets/support_messages (không phải UUID), fix migration để match. CI có thể cần user retry để confirm pass qua dashboard Actions. |
| D77 | Model alias map fix + dynamic NCC model fetch (2026-08-12) | ✅ **DONE** (2026-08-12) | Phát hiện lỗi: `kandes-claude` alias map sang `claude-sonnet-4.6` (DẤU CHẤM) nhưng NCC Pro upstream thật là `claude-sonnet-4-6` (DẤU GẠCH NGANG) → mọi request từ `kandes-claude` fail 404. Fix: (1) `modules/ai-gateway/models.ts` update upstream names đúng format hyphen cho tất cả Claude models (`claude-sonnet-4-6`, `claude-opus-4-6`, `claude-haiku-4-5`). (2) Thêm `listModels(apiKey)` vào `CcProProvider` để fetch live model list từ NCC `/v1/models` (cache 5 phút). (3) Export `listModelsFromCcPro()` standalone function. (4) `app/api/ai/v1/models/route.ts` — nếu Bearer token match `sk-jy-(cx\|cc)-*` pattern → fetch models từ NCC Pro real-time (trả `owned_by: 'ccpro'`); nếu `ks-*` (Kandes alias) → trả alias map như cũ. (5) `app/api/me/ai-keys/[id]/balance/route.ts` thêm `availableModels[]` + `modelStats[]` (từ NCC `/v1/usage`) + `nccRemaining`/`nccExpiresAt`/`nccDaysUntilExpiry`/`nccMode` real-time. Files: 5 files changed. Verified: `kandes-claude` → upstream `claude-sonnet-4-6` returns 200. `GET /api/ai/v1/models` (Bearer NCC key) returns 19 models: 5 Sonnet, 6 Opus, 2 Haiku, 1 Fable, 4 cũ. Build pass, deploy success. |
