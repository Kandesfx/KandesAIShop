...
| Phase 8b: Storefront Purchase Flow UI Cleanup | 🔄 **In progress** (2026-08-07) | Audit storefront (Landing → Catalog → PDP → Cart → Checkout → Order → Track) → 47 findings (A1..F6) chia 5 đợt ưu tiên. Đợt 1 unblock purchase flow (A1 AddToCartButton + A5 variant selector + A6 bỏ name state + A3 router.refresh + B10 footer link + C10 stale comment). Spec: `docs/tasks/PHASE_8_STOREFRONT_CLEANUP.md`. Plan: `docs/tasks/PHASE_8_PLAN.md`. Quy tắc: KHÔNG tự ý đổi thứ tự đợt, deviation mới ghi vào §7 D65+. |
