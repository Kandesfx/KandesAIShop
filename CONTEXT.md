...
| D76 | Cleanup sweep (D74 batch A–F) | ✅ **DONE** (2026-08-12) | (A) Fix 2 cron AI fail — 13/13 migrations applied (DB schema up to date), 6/6 cron pass. (B) Cron backup rclone Google Drive — `rclone 1.74` installed, `/etc/cron.d/kandes-db-backup` schedule 03:30 UTC daily, `scripts/aws/backup-db-rclone.sh` ready, `docs/deployment/BACKUP_RCLONE.md` — user cần chạy `rclone config` thủ công để OAuth Google Drive. (C) CloudFront for `api.kandes.shop` — distribution #2 deployed (D75a). (D) Xóa 4 secrets unused: `kandes/JWT_SECRET`, `kandes/REFRESH_SECRET`, `kandes/NEXTAUTH_SECRET`, `kandes/NEXTAUTH_URL` (force-delete-without-recovery). (E) Xóa ECR repo orphan `kandes-shop` (empty, unused since D66). (F) Fix CI — `prisma/migrations/20260805030000_add_user_notification_prefs/migration.sql` chuyển `"user"` → `"users"` + `IF NOT EXISTS` guards để fresh CI DB không fail. Pushed commit `0ba72a3` — chờ CI chạy. |
