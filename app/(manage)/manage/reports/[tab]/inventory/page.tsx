        ...
        <table className="w-full text-[12px]">
          ...
          <tbody className="divide-y divide-ink-400/30">
            {report.byProduct.length === 0 ? (
              <tr>
                <td colSpan={6} className="p-6 text-center text-ink-200 text-[11px]">
              ...
              report.byProduct.map((p) => (
                <tr key={p.productId} className="hover:bg-ink-700/30">
                  <td className="p-3">
                    ...
                    <p className="text-[10px] font-mono text-ink-200">
                      {p.productSku}
                    </p>
                  </td>
                  <td className="p-3 text-right text-ink-200">{p.available}</td>
                  <td className="p-3 text-right text-ink-200">{p.reserved}</td>
                  <td className="p-3 text-right text-ink-200">{p.delivered}</td>
                  <td className="p-3 text-right text-ink-200">{p.total}</td>
                  <td className="p-3 text-center">
                    {p.isLowStock ? (
                      <span className="px-2 py-0.5 rounded bg-warning/20 text-warning text-[10px] font-mono">
                        Low stock
                      </span>
                    ) : (
