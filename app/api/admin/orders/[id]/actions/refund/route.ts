 ...
 *
 * Body: { amountCents: string, reason: string }.
 * Phase 3 (D27): chỉ ghi nhận trạng thái nội bộ — KHÔNG gọi SePay API.
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await refundOrder(id, body, { id: user.id, role: user.role })
    return ok(result)
  } catch (err) {
    return fail(err, req)
  }
