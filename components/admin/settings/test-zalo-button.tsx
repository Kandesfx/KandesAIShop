...
export function TestZaloButton() {
  ...
  async function send() {
    ...
    setResult(null)
    try {
      const resp = await fetch('/api/admin/settings/test-zalo', {
        method: 'POST',
  ...
  async function checkOA() {
    ...
    setResult(null)
    try {
      const resp = await fetch('/api/admin/settings/test-zalo', { method: 'GET' })
      const data = (await resp.json().catch(() => ({}))) as {
        ok: boolean
        data?: { oa: { name: string } }
        error?: { message: string }
      ...
      setResult({ ok: true, message: `OA OK: ${data.data?.oa.name}` })
    } catch (err) {
      setResult({ ok: false, message: err instanceof Error ? err.message : 'Lỗi' })
    } finally {
      setBusy(false)
